...
def parse_args():
    ...
    ap.add_argument('--q68', action='store_true',
                    help='train the outer quantiles at 0.1585/0.8415 instead of 0.25/0.75, so the '
                         'interval the loss shapes is the one sigma_eff measures. The metric is '
                         'the half-width of the 68.3%% interval; the objective has always shaped '
                         'the 50%% one, and 18.3%% of events sit between the two contours -- the '
                         'loss stops caring about them while the metric still counts them. The '
                         'measured 68.3/50 ratio is 1.59 against a gaussian 1.48, so that band is '
                         'heavier than a matched loss would leave it.')
    ap.add_argument('--gs-balance', action='store_true',
                    help='average the gate-supervision loss WITHIN each region before averaging '
                         'across regions. One pooled mean lets whichever region supplies the most '
                         'labelled cells own the gradient: the overlay is 36%% 60mm and 0%% 120mm, '
                         'and 120mm is the one region that got worse when the others improved.')
    ...
    ap.add_argument('--cellreg', action='store_true',
                    help='regress the photon ENERGY in each cell instead of a weight on the '
                         ...
                         'cell that sampled low or fell under the 2.49 MeV threshold cannot be '
                         'recovered. The error budget says the dominant term in the highest-'
                         'leverage bin is an energy-independent 0.29 GeV, an ADDITIVE error that '
                         'a multiplicative estimator cannot express however it is trained. The '
                         'residual head is zero-initialised, so this starts exactly at the gated '
                         'sum and must earn any departure.')
...
def train_one(T, D, seed, args, device):
    ...
    qs = torch.tensor([0.1585, 0.5, 0.8415] if args.q68 else QUANTILES, device=device)
    ...
    def loss_fn(qa, yb, b=None):
        ...
        if args.gatesup > 0 and b is not None and getattr(model, 'last_w', None) is not None:
            ...
            if hf.sum() > 0:
                ...
                se = (model.last_w - T['FR'][b]) ** 2 * mm
                if args.gs_balance:
                    # One pooled mean lets whichever region contributes the most labelled cells
                    # own the gradient. The overlay is 36% 60 mm and 0% 120 mm, and 120 mm is
                    # exactly where the region-complete overlay LOST ground (+0.0033 at low-E)
                    # while every other region gained -- the supervision was competing with
                    # itself. Averaging within a region first, then across regions, gives each
                    # the same say regardless of how many of its cells happen to be labelled.
                    # the region one-hot is channels 11..15 of every token (Eq. 1), so the event's
                    # region can be read off the first live token rather than threaded through
                    # the whole data pipeline
                    first = T['M'][b].float().argmax(1)
                    oh = T['X'][b][torch.arange(len(first), device=first.device), first, 11:16]
                    reg = oh.argmax(-1)
                    if reg is not None:
                        tot, n = 0.0, 0
                        for r in range(5):
                            ...
                            if k.any():
                                mk = mm[k]
                                if mk.sum() > 0:
                                    tot = tot + se[k].sum() / mk.sum().clamp(min=1.0)
                                    n += 1
                        d2 = tot / max(n, 1)
                    else:
                        d2 = se.sum() / mm.sum().clamp(min=1.0)
                else:
                    d2 = se.sum() / mm.sum().clamp(min=1.0)
...
def main():
    ...
    if args.gatesup > 0 or args.synaux:
        # comma-separated paths concatenate: the same clean photons paired with independent
        # pileup draws. Supervision is the only lever that has worked and it currently reaches
        # 39% of the training set, so more of it is the cheapest thing left to try -- and unlike
        # real data we can generate it.
        syn_ev = []
        for path in str(args.overlay).split(','):
            with open(path.strip(), 'rb') as f:
                syn_ev.extend(pickle.load(f))
