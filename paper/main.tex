...
\paragraph{Tokens.} Each calorimeter cell is a token of sixteen channels: its
total, front and back energies as $\log(1+E)$; its offset from the window
centre in cells, $(\Delta i, \Delta j)$, and the radius $r$; the logarithm of
the cell pitch; its front and back timestamps with the window median
subtracted and clipped to $\pm 5$\,ns; two has-timestamp indicators; and a
five-way region one-hot. The timestamps are the arrival times themselves, not
an engineered pull --- the median subtraction only removes the event's overall
time offset. Cells without a usable timestamp carry the indicator alongside a
zero, which after the linear embedding is equivalent to a learnable
