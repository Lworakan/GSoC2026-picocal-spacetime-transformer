...
photon carries on average only $30\%$ of its cluster's energy. SpaceTformer,
...
model. The starting window held under half of the innermost region's cluster
...
capacity sweep and a recipe sweep all measured null against the same bins. A directly measured
...
the sample used here the photon carries on average $30\%$ of the energy
inside its own cluster, with the contamination worst in the small-cell inner
regions. Reconstructing the photon energy then stops being a calibration
problem and becomes a separation problem.
...
hypothesis to be measured. Roughly 110 configurations were measured this way
...
window, not the model --- fixing the window's containment and centring cut
the worst bin by $53\%$, while every architectural alternative measured
null, leaving training-set size as the only open lever.} The paper presents
the headline result and its uncertainty protocol
(Section~\ref{sec:primary}), the diagnosis with its mechanism measurements
(Sections~\ref{sec:gain}--\ref{sec:anatomy}), the controlled architecture
campaign (Section~\ref{sec:encoders}), the ledger of measured negative
...
with severe per-cell noise, local graph connectivity buys nothing over full
attention.
...
time axis wins in video; our time-sliced attention arm transplants that idea
...
Patch-hierarchical attention for jets~\cite{patchjets} motivated our
ring-summary window extension (fine detail in the core, coarse summaries
outside), the one transplanted idea that survives measurement.
...
Deep Sets~\cite{deepsets} and Energy Flow Networks~\cite{efn} pool
per-particle embeddings additively; our EFN arm confirms that additive
pooling is too weak when the task is subtractive --- separating overlapping
energy fields rather than summarising one. MLP-Mixer-style token
mixing~\cite{mixer} replaces attention with fixed linear mixing across
tokens; a masked Mixer arm measured here loses heavily
(Section~\ref{sec:encoders}) --- under pileup the informative cells move
event by event, which content-based attention tracks and fixed mixing
weights cannot.
...
benefit appears at long sequence length (published comparisons concern
thousands of tokens). Our events carry 81--289 tokens, where full attention
is exact and cheap, so we deprioritised the family without measurement and
say so explicitly.
...
calorimeter energy regression at CMS uses dynamic graph
reduction~\cite{drn} and deep clustering in the ECAL~\cite{cmsecaldl}.
Those works subtract pileup or regress energy; ours does both in one model
and reports where the ceiling comes from. The CMS HGCAL~\cite{hgcal}
programme reports the same qualitative pattern we find --- pileup-aware
reconstruction gains concentrate at low energy and in the fine-granularity
region --- which supports reading our per-bin results as physics rather
than tuning. The quantile (pinball) loss goes back to~\cite{koenker}, its
neural form to~\cite{qrnn}; exponential weight averaging
to~\cite{polyak}; deep ensembles to~\cite{deepens}.

\section{Dataset and task}
\label{sec:data}

Two simulated samples are used. The \emph{minimum-bias sample} --- the
physics target and the source of every headline number --- contains 72{,}554
...
compartments --- only $\sim$$30\%$ of cells carry a usable timestamp ---
...
split with larger seed ensembles; that protocol flatters the weak bins by
roughly $0.009$ (15\,mm low-E: $0.0691$ fixed-split vs $0.0783$
cross-validated), so we do not quote fixed-split numbers as headlines and we
label the protocol wherever one appears. Against selection effects across
the $\sim$120-configuration campaign (Appendix~\ref{app:ledger}): the
headline protocol was fixed before the final measurement, screening runs
never enter headline claims, and the half-sample preview and its correction
...
which is the honest reproducibility scale --- a pooled statistical error
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{spacetformer.pdf}
\caption{The SpaceTformer pipeline. Each cell is a token in detector space
and time; the encoder is a plain transformer; the readout is a gated energy
sum in log space; the loss is a three-quantile pinball loss with EMA
weights; predictions are the per-event median over ensemble members. The
two decisive components are on the far left: window size and window
centring (Section~\ref{sec:gain}).}
\label{fig:arch}
\end{figure}

\paragraph{Tokens.} Each calorimeter cell is a token carrying its energy,
local coordinates relative to the window centre, and its raw front and back
timestamps; cells without a usable timestamp carry explicit has-timestamp
indicator channels alongside the zero-filled value, which after the linear
embedding is equivalent to a learnable missing-value embedding.
Cluster-level reconstruction quantities enter as global features; all
inputs come from the standard reconstruction and none uses generator truth.
Figure~\ref{fig:input} shows one real input: the pileup blanket the model
must subtract, the sparse timestamps, and the three centre candidates the
window story of Section~\ref{sec:gain} turns on.
...
configuration uses a $17{\times}17$ window centred on the cluster energy
...
$4d$, dropout 0.1, multi-head self-attention over cells with the global
features injected. The name marks what the model attends over --- each token
is a point in detector space \emph{and} time, with the raw timestamps worth
...
gated readout, Huber & first structural win & 0.0462\\
...
exponential moving average of the weights (decay 0.999)~\cite{polyak} used
for evaluation. A six-arm sweep of this recipe at the end of the project
...
Table~\ref{tab:primary} is the headline table; Figure~\ref{fig:resve} shows
...
per region. The fit tells the physics story compactly: the $1/E$ term ---
...
\subsection{The diagnosis: window containment and window centring}
\label{sec:gain}
...
cluster energy versus window half-width, per region. At $w{=}4$ the
...
silently tuned for the abundant outer regions. The physics behind the
asymmetry: the shower has a single physical width ($99.7$--$100\%$ of photon
energy within 120\,mm in every region on the clean sample), so its width
\emph{in cells} varies eightfold across regions.
...
15\,mm low-E falls monotonically from $0.186$ to $0.102$ as the window
opens --- the single largest lever found in the whole project --- while the
\emph{aggregate barely moves} ($0.038$--$0.044$ across the whole scan),
...
headline metric hid it. Ring summaries (coarse sums of the cells outside
the window, after~\cite{patchjets}) let $w{=}8$ match $w{=}10$ at half the
...
worst bin halves: the headline metric hid the defect.}
...
it invites a wrong story --- that the barycentre finds the photon better
than the seed cell. That story is measurably false.
...
the barycentre-centred window wins. The window is not a photon-pointer; it
...
\midrule
$9{\times}9$ seed-centred window & 0.1654 & fixed split & 41\% containment (Fig.~\ref{fig:containment})\\
widen to $17{\times}17$ & 0.1156 & fixed split & coverage\\
+ re-centre on barycentre & 0.0711 & fixed split & coverage again (see text)\\
cross-validated ensemble & 0.0783(22) & CV & headline protocol\\
\bottomrule
\end{tabular}
\end{table}
...
resolution --- direct evidence that the window's job is coverage of the
energy field, not photon localisation.}
\label{fig:misseed}
\end{figure}
...
tail-directed intervention moves the headline number --- but they matter
...
\subsection{Architectures: ten families, one survivor}
\label{sec:encoders}
...
SpaceTformer wins every cell at every seed. One fairness caveat is owed:
...
patch tokens (ViT-style) & \cite{patchjets} & --- & --- & cut at screening\\
...
\midrule
token transformer & \cite{vaswani} & \textbf{0.0390} & & the survivor\\
\bottomrule
\end{tabular}
\end{table}
...
\emph{trains} and still buys nothing. At 81--289 tokens per event, full
attention is not the bottleneck, and nothing cheaper matches it.

\subsection{The gate is not a fraction estimator}
\label{sec:gate}
...
The result independently supports the case for dedicated precision-timing
layers at the HL-LHC~\cite{mtd}: every picosecond of per-cell timing
resolution feeds a model like this one directly.

\subsection{Closed doors}
\label{sec:closed}

Table~\ref{tab:closed} is the ledger of measured nulls beyond the encoder
...
neutral-or-worse, each with its row in Appendix~\ref{app:ledger}: time-pull
...
configuration; its one win pre-recentring was superseded by recentring
...
Four limitations bound the claims. (1) Everything is measured on one
...
window centring. Everything else we tried is reported with its numbers in
Sections~\ref{sec:encoders}--\ref{sec:closed} and
Appendix~\ref{app:ledger}, so it does not have to be re-tried silently.
The measured scaling curve, $\seff \propto N^{-0.18}$, makes the path below
$\seff = 0.035$ a data request rather than an architecture search.
...
Table~\ref{tab:ledger} lists every configuration with a saved prediction
set on the minimum-bias sample, scored by the same seed-ensemble \seff{}
(median prediction per event across seeds, then one \seff). The table is
generated from the prediction files by the same script that produced every
number in the body; nothing is transcribed by hand. Configuration names
compose tags: \texttt{W$n$} window half-width, \texttt{CleanAux}
clean-sample auxiliary, \texttt{Ex} extra globals, \texttt{Dn} density,
\texttt{Qd} quantile loss with width term, \texttt{Ema} weight averaging,
\texttt{Rc} recentred window, \texttt{Rr01} recentring restricted to the
15/30\,mm regions, \texttt{Ac} all cluster cells, \texttt{K$n$}
cross-validation fold $n$, \texttt{R$n$} ring summaries to ring $n$,
\texttt{Tp} time pulls, \texttt{Gs}/\texttt{Pf}/\texttt{Pt} gate
supervision variants, \texttt{Mm} millimetre geometry, \texttt{Tc}
front/back time combination, \texttt{Lr}/\texttt{B}/\texttt{Cos}/\texttt{Cj}
recipe-sweep arms, \texttt{Tr} trimmed risk, \texttt{NoTime} timestamp
ablation.
...
using Transformers (ClusTEX)}, arXiv:2603.18172 (2026).
...
\bibitem{dinov3} O.~Sim\'eoni et al.,
...
\emph{Framework TDR for the LHCb Upgrade II},
...
LHC Run 2 data}, arXiv:2309.05471 (2023).
