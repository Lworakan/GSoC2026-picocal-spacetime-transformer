...
models that never saw it). Scored against a six-feature gradient-boosted
reference on the events the two share, the improvement is a factor
$3.15 \pm 0.05$, with the tree behind in every one of $400$ paired
resamples and in every region separately. In a paired comparison with
...
families, five timing constructions, a capacity sweep and a recipe sweep
all measured null against the same bins. Gate supervision belongs on the
other side of that line, and only later measurement put it there: four
protocols measured null on an overlay that covered two of the five regions,
and the same idea wins once the overlay is region-complete and the readout
regresses a per-cell energy rather than a multiplicative gate
(Sec.~\ref{sec:gate}).
...
Upgrade~II~\cite{lhcbftdr}. It is a sampling calorimeter built from
$12 \times 12$\,cm modules of two technologies --- a spaghetti calorimeter
close to the beam and shashlik further out --- segmented so that the cell
pitch grows with distance from the beam pipe: five regions at $15$, $30$,
$40$, $60$ and $120$\,mm, from one to 64 cells per module. Every cell is read
out at both depths and carries a timestamp. The design intent of that
segmentation is to keep the occupancy per cell manageable where the particle
flux is highest, which is where the beam pipe is.

It also sets the shape of the problem this paper is about. At the luminosities
Upgrade~II targets
\todo{state the simulated $\nu$ / luminosity --- ask the collaboration},
every photon cluster carries energy from tens of simultaneous
proton--proton interactions. In the sample used here the photon's median share
of the energy inside its own cluster is $58\%$ over all regions but only
$19\%$ at 15\,mm and $32\%$ at 30\,mm. The gradient follows the beam pipe, not
the cell size: the inner regions are contaminated because the particle flux
there is highest, and the fine segmentation is the response to that flux
rather than its cause --- a coarser inner calorimeter would be contaminated
more, not less. Reconstructing the photon energy in those regions then stops
being a calibration problem and becomes a separation problem. Figure~\ref{fig:pvp} makes the
...
GravNet~\cite{gravnet}, attention models~\cite{part}, and hybrids. One of
them addresses this detector and this task directly: a graph network for
energy reconstruction in the Upgrade~II electromagnetic
calorimeter~\cite{vetens}, built on an attention-enhanced GarNet and
compressed for real-time use. We could not compare against its numbers, which
are not published, so what we can say is narrower and we say only that: the
GarNet family's closest published relative, GravNet, was re-implemented here
behind an identical readout, loss, splits and window, and reads $0.0437$
against the transformer's $0.0399$ as single models on the same events. That
is a statement about the architecture in our pipeline, not about their result.
Papers in
...
particle list under a fixed sequence ordering ($k_\mathrm{T}$ in their
experiments), explicitly not spatial
regions. We
...
premise as our per-bin pattern: fine transverse granularity is there to
resolve showers in a dense environment~\cite{hgcal}, and per-cell timing at
the tens-of-picoseconds level is there so that deposits from pileup vertices
can be rejected~\cite{hgcaltiming} --- which is where our own improvements
concentrate. The quantile (pinball) loss goes back to~\cite{koenker} and was carried
...
Figure~\ref{fig:resve}. The terciles are computed per region and are
therefore not comparable across regions: the median photon energy falls from
$59.8$\,GeV at 15\,mm to $12.5$\,GeV at 120\,mm, so the 15\,mm ``low''
bin ($20$--$47$\,GeV) sits above the entire 120\,mm spectrum. Rows of
Table~\ref{tab:primary} should be read down a column, not across one.
...
the $134$-configuration campaign (counted as in the introduction,
with cross-validated arms counted once; the scored ledger is versioned with
the code): the
...
\paragraph{GBDT reference.} The tree reference is one we trained ourselves, on
the same events and under the same protocol, rather than a number taken from
elsewhere; calling it external would suggest a published result being quoted,
and it is not. It is a
...
fixed split. It reaches aggregate $\seff = 0.1253$ on its own test set
(per-region curves in Figure~\ref{fig:resve}). That set is neither the
development split nor the cross-validated sample, so the two numbers are not
divided directly; instead both estimators are scored on the $10{,}865$ events
they share, where the tree reads $0.1189$ against the ensemble's $0.0377$, a
factor $3.15 \pm 0.05$ with the tree behind in $400$ of $400$ paired
resamples. The comparison holds cell by cell and not only on the
aggregate: Table~\ref{tab:vsbdt} scores both estimators in all fifteen
region--energy bins, and the transformer is ahead in every one of them, by
between $2.08\times$ and $4.06\times$. There is no bin in which the tree is
competitive, so the aggregate ratio is not an average over a mixed result.
It is a fast-baseline reference,
not a tuned competitor, and we label it as such.

\begin{table}[h]
\centering
\caption{\seff{} in all fifteen region--energy bins, both estimators scored
on the $10{,}865$ events they share. Terciles are per region and are not
comparable across rows. The transformer is ahead in $15$ of $15$ bins.}
\label{tab:vsbdt}
\small
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{lccccccccc}
\toprule
& \multicolumn{3}{c}{low-E} & \multicolumn{3}{c}{mid-E} & \multicolumn{3}{c}{high-E}\\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
region & ours & GBDT & gain & ours & GBDT & gain & ours & GBDT & gain\\
\midrule
...
\bottomrule
\end{tabular}
\end{table} One substitution relative
...
token. Sixteen channels are the detector reading itself,
\begin{equation}
\mathbf{x}^{0}_i = \Big(
  \log(1{+}E_i),\;
  \log(1{+}E_i^{\mathrm{f}}),\;
  \log(1{+}E_i^{\mathrm{b}}),\;
  \delta^u_i,\; \delta^v_i,\; r_i,\;
  \log p,\;
  \Delta t_i^{\mathrm{f}},\; \Delta t_i^{\mathrm{b}},\;
  m_i^{\mathrm{f}},\; m_i^{\mathrm{b}},\;
  \mathbf{1}_\rho
\Big) \;\in\; \mathbb{R}^{16},
\label{eq:token}
\end{equation}
where $E_i$ is the cell energy and $E_i^{\mathrm{f}}, E_i^{\mathrm{b}}$ its
front and back longitudinal samples; $(\delta^u_i, \delta^v_i)$ is the offset from
the window centre in cells and $r_i$ its Chebyshev modulus; $p$ is the cell pitch;
...
$m_i^{\mathrm{f}}, m_i^{\mathrm{b}} \in \{0,1\}$ record whether each timestamp
...
usable timestamp carries $m=0$ beside a zero, which after the linear embedding
is equivalent to a learnable missing-value embedding.

Every member of the primary result carries five further channels, added
because the ablation of Section~\ref{sec:closed} measured them worth
$0.0409 \to 0.0389$ together: the cell's offset from the reconstructed
cluster centroid in units of the pitch, $(x_i-x_c)/p$ and $(y_i-y_c)/p$, and
the same three energies again normalised for the cell area,
$\log(1{+}E_i) - 2\log(p/15\,\mathrm{mm})$ and its two longitudinal
counterparts, so that a deposit means the same thing in a $15$\,mm cell as in
a $120$\,mm one. The token the encoder actually embeds is therefore
$\mathbf{x}_i \in \mathbb{R}^{21}$, and a reader reproducing
Table~\ref{tab:primary} needs all twenty-one.
The whole set $\{\mathbf{x}_i\}$ is what the encoder attends over; nothing is
aggregated before it.
Cluster-level reconstruction quantities enter as global features. There are
fourteen: five describing the window itself ($\log(1{+}\sum_i E_i)$, the seed
energy, $\log n$ for $n$ fired cells, the front/back energy ratio, and the
energy-weighted lateral width), eight relating the window to the cluster the
standard reconstruction found (its total energy, the fraction of it the window
captures, the same fraction front and back separately, the cluster's own
front/back ratio, the number of reconstructed clusters in the event, and the
two components of the window centre's offset from the cluster centroid), and
one indicator marking events drawn from the clean auxiliary sample rather than
from minimum bias. All inputs come from the standard reconstruction or from
that sample label; none uses generator truth.
...
($15$\,mm region, $E_\gamma = 37$\,GeV, 424 fired cells in the cluster,
of which the window admits at most 289). (a)~Each fired
...
cluster ($E_\gamma = 37$\,GeV, 424 fired cells in all, of which the
$17{\times}17$ window admits at most 289) shown in the recentred window. Left: per-cell energies --- pileup deposits
...
fourteen cluster-level global features do not enter the encoder; they are
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/resolution_vs_et.pdf}
\caption{The same per-region curves against \emph{transverse} energy, the
axis in which the physics selections are written. Single model, seed~0,
development split; dashed is the starting configuration and solid this work.
The ordering between the two is the same in every region and at every point,
which is the property an axis change should not disturb and does not.}
\label{fig:resvet}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/design_gap.pdf}
\caption{The same curves with PicoCal's own specification drawn on top:
$\sigma_E/E = 10\%/\sqrt{E} \oplus 1\%$~\cite{picocalpos}, dotted, evaluated
at each bin's median true energy. The red line is $0.08$. The detector splits
in two. At 60 and 120\,mm the measured points converge onto the design curve
at the top of each region's range --- within $6$--$17\%$ of the number the
calorimeter was specified to, on a sample that carries pileup the
specification does not --- and the design curve is itself above $0.02$ there,
so no estimator reaches $0.02$ in those regions at the energies they see. At
15 and 30\,mm the curve sits a factor $1.9$--$2.7$ above a design floor of
$0.015$--$0.018$, and since the design stochastic term is $0.10$ against a
fitted $0.21$, that excess is pileup rather than sampling. Energy and
pseudorapidity are tied at fixed transverse energy, which is why the outer
regions never reach the energies where the inner ones are measured.}
\label{fig:designgap}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/perbin_bars.pdf}
\caption{Every region--energy cell, single model at seed~0 on the
development split: per-cell energy regression with gate supervision on the
region-complete overlay against the same recipe without them. Fifteen of
fifteen cells improve and all fifteen sit below $0.08$, the worst being
30\,mm low at $0.0712$. Cross-validation of this configuration was
incomplete at the time of writing; see Sec.~\ref{sec:gate}.}
\label{fig:perbinbars}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/vs_gnn.pdf}
\caption{The same resolution against \emph{transverse} energy, which is the
axis the graph-network work on this detector reports~\cite{vetens} and the
one in which the physics selections are written. Left: the 30\,mm region,
which is the SpaCal-Pb equivalent --- their shashlik modules are $4 \times
4$\,cm, our 40\,mm, and SpaCal-Pb sits between the tungsten spaghetti
calorimeter and the shashlik. Right: all regions. Dashed is the starting
configuration, solid this work. Plotted this way the two studies can be laid
side by side: the metric is the same $\seff$, and this sample already lies
inside their $0.5$--$5$\,GeV range. It is an axis, not a claim about their
numbers --- their photons are isolated with pileup overlaid rather than real
minimum-bias events, and their pileup condition is the worst case rather than
the nominal one.}
\label{fig:vsgnn}
\end{figure}

\paragraph{Why the curve stops falling at the top.} A resolution-versus-energy
curve on this sample is not one detector measured at several energies: it is a
different mixture of regions at each point. Below 10\,GeV the sample is 60\%
60\,mm and holds no 15\,mm clusters at all; above 80\,GeV it is 58\% 15\,mm and
42\% 30\,mm, with nothing from the outer regions. Energy and pseudorapidity are
tied together at fixed transverse energy, so climbing the axis walks inwards
across the calorimeter and into the worst of the pileup. The flattening above
about 20\,GeV, and the point-to-point wobble past 80\,GeV --- $2.49\%$ at
80--90\,GeV against $2.80\%$ at 90--100\,GeV, a $1.5\sigma$ difference on
statistical errors of $0.13$ and $0.19\%$ --- are that composition changing
under the curve, not the estimator degrading. Read the per-region curves for
detector behaviour and the aggregate curve for what a mixed sample delivers.
...
The fit quality has to be stated with the parameters: $\chi^2/\mathrm{ndf}$
is $1.0$ at 120\,mm, $1.6$ at 60\,mm, $2.3$ at 40\,mm and $3.9$ at 30\,mm,
but $23$ at 15\,mm, where the stochastic and constant terms both run onto
their bounds. The three-term form is a poor description of the innermost
region, and the $b = 2.1$\,GeV there should be read as a statement that the
$1/E$ term dominates, not as a measured noise constant.

\paragraph{Which baseline is \emph{the} baseline.} Three reference points appear
in this paper and they are not interchangeable, so we state plainly what each
one is for. The \emph{direct} comparison point --- the thing a new
reconstruction has to beat before it is worth anything to the collaboration ---
is the standard $3\times3$ cluster reconstruction on the same events, because
that is what LHCb runs today and what the Upgrade~II detector will run on day
one. The gradient-boosted reference is a control we trained ourselves, matched
in events, splits and protocol, and it exists to separate "the transformer
helps" from "any learned estimator helps"; it is not a collaboration baseline
and is not offered as one. PicoCal's design specification,
$\sigma_E/E = 10\%/\sqrt{E} \oplus 1\%$~\cite{picocalpos}, is the third and is
neither a competitor nor a control: it is the floor, and the useful question
against it is not whether we beat it --- on a pileup-loaded sample we should
not --- but how far above it each region sits (Fig.~\ref{fig:designgap}).

\paragraph{Against the calorimeter LHCb has now.} The natural external scale is
the existing LHCb electromagnetic calorimeter, whose simulated resolution runs
from about $11\%$ at low energy to about $4\%$ at very high energy, integrated
over its areas~\cite{lhcbcalo}. Binned the same way, this work reads
$8.0\%$ below 5\,GeV, $4.1\%$ at 10--20\,GeV, $3.0\%$ at 30--50\,GeV and
$2.6\%$ above 80\,GeV, on a minimum-bias sample --- that is, \emph{with} the
pileup, which the reference curve does not carry.

Three things make this a scale rather than a like-for-like comparison, and we
state them rather than let the numbers stand alone. The detectors differ:
PicoCal is the Upgrade~II replacement, with finer granularity and per-cell
timing. The statistics differ: the reference is a Gaussian width, ours the
half-width of the 68.3\% interval, which is the more conservative of the two
on a distribution with tails. And the tasks differ: the reference measures a
calibration chain, ours a regression trained on the same simulation it is
scored against. What the comparison does establish is that the numbers here
sit at the scale of a working calorimeter's performance rather than beside it.
...
One term needs pinning down before the comparison, because it is easy to read
the wrong way. The recentred window is placed on the centre of the seed
\emph{module} --- the $12 \times 12$\,cm block that holds between one and 64
cells --- not on the centre of the seed \emph{cell}. In the 15\,mm region a
module is eight cells across, so the two differ by up to four cells, and it is
the module centre, not the cell, that the measurements below are about.

That choice is a trade the paper should state rather than sell. Anchoring the
aperture to the module buys coverage at low energy in the inner regions, where
the pileup-dragged barycentre is the problem; it costs position resolution,
because the frame the network sees is quantised to the module and no longer
follows the photon. This work measures energy, so the trade is worth taking
here; an analysis that needs the photon's position would want the opposite.
...
$0.0389 \pm 0.0003$ --- the five best single members of the
...
pooled across the three families & 20 & 0.0376 & $-0.0013 \pm 0.0002$, $P{=}1.00$\\
\bottomrule
\end{tabular}
\end{table}
...
($-0.0002 \pm 0.0003$, $P{=}0.65$ between them): both recover the same
...
development protocol on the pooled 28-member ensemble ($0.0376$ aggregate,
...
bound on what any improvement to that bin can buy. The rows requiring
per-cluster quantities are computed on the events whose energy key joins the
geometry cache, $46\%$ of the split; that matched subset is not
representative --- it scores $0.0427$ against the full split's $0.0376$ ---
so the shares and headrooms below describe the harder half and should be
read as indicative rather than exact. ``Sampling part'' is
$a/\sqrt{E}$ from a per-region fit of
$\seff(E) = \sqrt{a^2/E + b^2/E^2 + c^2}$, the parameterisation of
Section~\ref{sec:primary} written out, evaluated at the bin's median
...
$0.0225$ against a measured $0.0513$ --- and there the dominant term is
$b/E = 0.0387$, an absolute error of $b \approx 0.29$\,GeV at that bin's
...
mean is $0.0394$ and the best $0.0386$; pooling all 28 gives $0.0376$, only
...
SpaceTformer performs best in every cell at every seed. Two caveats are
required for fairness. The first is capacity: behind the identical readout,
the transformer encoder holds $625$k parameters against ParticleNet's $180$k
and GravNet's $279$k, so it is the larger model by a factor $2.2$--$3.5$ and
the comparison is at equal \emph{recipe}, not equal capacity. We did not
grow the baselines to match, and the honest reading is that part of the
margin may be capacity rather than architecture. What speaks against that
reading is our own capacity sweep on the transformer itself
(Table~\ref{tab:closed}): $d{=}256$ with six layers scores $0.1179$ and
$d{=}64$ with two layers $0.0463$, both far worse than the $d{=}128$
baseline, so on this sample parameter count is not a lever that buys
resolution --- which is also why the baselines were not given more.
The second caveat:
...
Because only the encoder differs, the comparison can be paired event by
event rather than read off seed spreads. Scoring every baseline seed against
every transformer seed on the same events, and resampling events once per
draw so both are scored on the same resample, the default ParticleNet is
behind in at least $393$ of $400$ resamples in its worst seed pairing (ten
pairings) and GravNet in $400$ of $400$ (ten pairings). Of the six tuned
arms, five are behind in $400$ of $400$. The exception is the best of them,
ParticleNet at lr $6\times10^{-4}$: its worst seed pairing beats the
transformer in $46$ of $400$ resamples on the aggregate, so on the aggregate
alone that single arm is not separated. It remains behind in the two weak
bins ($0.0809$ against $0.0775$ pooled over 15 and 30\,mm low-E) at
$3.6\times$ the training cost. This is the sharpest form of the claim the
evidence supports, and it is weaker than ``wins everywhere'' on the
aggregate for that one arm.
...
\seff{} with the era's matching transformer reference. Values are
single-seed unless an arm was run with more, in which case the row is that
arm's seed ensemble --- the time-sliced row is a three-seed ensemble
($0.0456$ as a single-seed mean) while the Mixer row is one seed; the
column is therefore not uniform in seed count and the rows should be
compared with their own era reference rather than with each other. This is
also why the ParticleNet/GravNet rows differ slightly from
...
\paragraph{The null was a property of the overlay, not of the idea.} Both
results above were measured on an overlay that covered the 15 and 30\,mm
regions only, which is $31\%$ of the sample: the remaining $69\%$ of the
training events carried no per-cell label at all. Regenerating the overlay
across all five regions and supervising a readout that regresses a per-cell
\emph{energy} --- one that may exceed the cell's own deposit, rather than a
multiplicative gate in $(0,1)$ that cannot express an additive error ---
reverses the conclusion. On the development protocol the combination reads
$\seff = 0.0376$ as a single model against the same recipe's $0.0399$, and
$0.0367$ against $0.0388$ as a three-seed ensemble, ahead of the control in
$15$ of $15$ region--energy bins with every region under $0.08$. A control
arm that adds the same synthetic events \emph{without} the per-cell loss
does not reproduce the gain, so what moved the number is the supervision and
not the extra events. Two things follow. The gate is still not a fraction
estimator --- the winning readout is the per-cell energy, not the gate ---
and a null measured under partial coverage is a statement about the
coverage. We report the earlier nulls and this result together rather than
replacing one with the other.

At the time of writing this configuration has four of ten cross-validation
folds complete, reading $0.0368$, $0.0369$, $0.0371$ and $0.0372$; the
headline of this paper remains the fully cross-validated $0.0388$ until all
ten are in. This project has twice had a development-split winner fail to
reproduce under cross-validation, and the number above is labelled
accordingly.
...
\label{fig:timing}
\end{figure}
We could not obtain the simulated timestamp resolution from the production
configuration, so we measured it on the samples themselves, taking the
energy-weighted time of a cluster's top-decile cells as the reference. It is
strongly energy-dependent: on the clean sample the per-cell spread falls
from $0.756$\,ns at $2$--$10$\,MeV to $0.435$, $0.263$, $0.156$ and finally
$0.038$\,ns above $3$\,GeV, a factor twenty across the range, and every bin
degrades by $1.7$--$2\times$ under pileup ($1.275 \to 0.132$\,ns over the
same cells). A cell's timestamp is informative only when the cell is
energetic --- which is why probe~(iii)'s ``quarter of the per-event spread''
is a tight constraint in the cells that carry the photon and a loose one in
the low-energy cells where the pileup sits, and why an explicit time gate
and a resolution-weighted pull both added nothing.
Every \emph{engineered} use of timing loses
...
and a Fourier basis on the raw time channels (a null: $0.0390$ against the
era's $0.0391$ at three seeds) were
...
timestamps are read, which the retraining ablation cannot. These perturb the
inputs of one trained member at evaluation and recalibrate per variant, so
they are scored against that member's own unperturbed $0.0402$ and not
against the ensemble of Table~\ref{tab:primary}; a model asked to read
corrupted timestamps it was trained to trust fails far worse than one
retrained without them, which is why the numbers below are an order of
magnitude larger than the $0.0485$ of the retraining ablation.
(i)~Permuting the timestamps among the cells of the same event
...
\midrule
gate supervision, 15/30\,mm overlay & 4 (direct, aux head, prior feature, distill) & neutral or worse; superseded, see Sec.~\ref{sec:gate}\\
...
Beyond these, fifteen per-cell and global feature mechanisms were measured
...
64, median of 5 timed passes on an idle machine). The transformer carries
$623{,}238$ parameters against the GBDT's $18{,}300$ tree nodes.
...
cluster than a single transformer pass and $3.15\times$ worse in resolution;
...
and for three-seed ensembles (exponent $-0.179$). The worst bin does not
merely follow: 15\,mm low-E runs $0.1254 \to 0.1017 \to 0.0956 \to 0.0729$
across the same points, an exponent of $-0.35$, about twice as steep as the
aggregate. More data helps the bin that needs it most, faster than it helps
the average.
...
ten-fold protocol --- a factor $3.15 \pm 0.05$ over a six-feature GBDT
reference on the events the two share --- and beats paired
re-implementations of ParticleNet and
...
PicoCal project. I thank my mentors Felipe Luan Souza de Almeida and Carla
Malin Benito for the simulated samples, weekly guidance, and the
error-analysis discipline this paper is built on. \todo{LHCb
...
together with a locked environment and a test suite whose regression canary
is \seff{} itself.

The script's defaults are the starting configuration, not the primary one,
so the three member families of Table~\ref{tab:primary} are named
explicitly. For fold $k \in \{0,\dots,9\}$:
\begin{verbatim}
...
\end{verbatim}
The five members of a fold are combined by per-event median; the ten folds
are concatenated. One gap should be stated plainly: the trained weights
behind the primary result were produced on rented cloud hardware and are not
in the repository, so a reader can re-score our predictions exactly but must
retrain to reproduce them from scratch.
...
\bibitem{vetens} W.~Vetens (LHCb Collaboration),
\emph{Graph Neural Network-based Energy Reconstruction for the LHCb Upgrade II
Electromagnetic Calorimeter}, ICHEP 2026, contribution 6959618.
\bibitem{lhcbcalo} LHCb Collaboration,
\emph{Calibration and performance of the LHCb calorimeters in Run 1 and 2 at the LHC},
arXiv:2008.11556.
\bibitem{picocalpos} \emph{The PicoCal of LHCb Upgrade 2},
PoS(LHCP2024)301, on behalf of the LHCb Collaboration.
\todo{author list to be completed from the published record}
...
\bibitem{hgcaltiming} A.~Lobanov (for the CMS Collaboration),
\emph{Precision timing calorimetry with the CMS HGCAL},
JINST \textbf{15} (2020) C07003, arXiv:2005.13324.
