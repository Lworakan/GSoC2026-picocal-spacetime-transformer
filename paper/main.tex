...
One architectural mechanism survives the campaign: a learned two-stage
window --- a first pass points at the photon (median error $0.16$ cells),
the window is re-cut around it, a second pass measures --- takes the
development-protocol ensemble from $0.0388$ to $0.0378$, better in every
one of $400$ paired resamples, and captures over half of the measured
bound that a perfectly centred window sets. A directly measured
...
hypothesis to be measured. One hundred and forty-eight configurations were measured this way, each with a
saved per-event prediction set (the complete scored ledger is versioned with
the code).
...
null except one that attacks the window itself (a learned two-stage
pointing window, Section~\ref{sec:pointing}), leaving training-set size as
the dominant open lever.} The paper presents
...
$p_\mathrm{T}$-ordered particle list, explicitly not spatial regions. We
...
Deep Sets~\cite{deepsets} pool per-particle embeddings additively and
Energy Flow Networks~\cite{efn} pool them in an energy-weighted sum, which
is what makes them infrared- and collinear-safe; our EFN arm confirms that
a linear pooling of this kind is too weak when the task is subtractive --- separating overlapping
...
and reports where the ceiling comes from. The design of the CMS HGCAL~\cite{hgcal} rests on the same premise as our
per-bin pattern: fine transverse granularity and per-cell timing were chosen
specifically so that overlapping deposits can be separated under HL-LHC
pileup, which is where our own improvements concentrate. The quantile (pinball) loss goes back to~\cite{koenker} and was carried
into neural networks by the forecasting literature~\cite{qrnn}; the
...
the $148$-configuration campaign (scored ledger versioned with the code): the
...
cluster-level global features enter at the readout, which is a gated energy
sum in log space plus a small correction; the loss is a three-quantile
pinball loss with EMA weights; predictions are the per-event median over
ensemble members. The decisive components are the window's size and
...
cluster-level global features do not enter the encoder; they are
concatenated to the pooled state at the readout, so attention operates on
per-cell information alone. The name marks what the model attends over --- each token
...
window gains through coverage while the seed is the better locator
...
Two caveats apply. First, all values in this subsection are
development-protocol numbers; promoting the two-stage result to a
primary result requires re-running the ten-fold protocol on it. Second, the
$2.6\%$ aggregate gain is small --- its significance is that it is the
\emph{measured majority of the measured maximum}: the true-entry bound
bounds the gain available to any centring mechanism at $4\%$; the
two-stage mechanism recovers over half of it, and no other architectural
change on this sample produced a measurable gain.
...
sample yields $\sim$$15\%$ --- what $N^{-0.18}$ purchases is diversity of pileup
...
one architectural mechanism that survives the campaign: the learned
two-stage window of Section~\ref{sec:pointing}, which points before it
measures and takes the development-protocol ensemble to $0.0378$,
capturing most of the margin a perfectly centred window is measured to
allow. Everything else we tried is reported with its numbers in
